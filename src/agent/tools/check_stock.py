...
class CheckStock(BaseTool):
    ...
    def description(self) -> str:
        return "查询商品库存和仓库信息。用户询问'有没有货''多久到'还有多少台'时使用。"
        ...
        return {
            ...
            "properties": {
                ...
                "table": {
                    ...
                    "description": "商品所在的数据库表，如果是商品是笔记本电脑填'laptop_products',手机填'phone_products'",
    ...
    def execute(self, product_name: str, table: str = "laptop_products") -> ToolResult:
        ...
        try:
            ...
            cur.execute(
                ...
                f"FROM {table} WHERE product_name ILIKE %s LIMIT 5",
                (f"%{product_name}%",),
            )
            rows = cur.fetchall()
            ...
            logger.error(
                ...
                product_name, table, str(e),
            )
            return ToolResult(
                name=self.name, status="error", error=f"库存查询失败: {str(e)}"
            )
